...
- [ ] 1.1 Add `session` domain records (pure Kotlin, no platform imports, by-layer package
...
- [ ] 1.3 Implement the application-level `SessionRegistryStorage` (dumb CRUD keyed by id)
...
- [ ] 1.4 Persist registrations (not state, not events) via `PersistentStateComponent` (flat
...
- [ ] 2.1 Implement the transport-agnostic `EventGateway` seam: resolve session by the id in
...
- [ ] 2.2 Implement the `HttpRequestHandler` extension for `POST
...
- [ ] 3.1 Add the `EnvironmentConfig` domain + app-level `PersistentStateComponent` store
...
- [ ] 3.2 Implement the launch service: register in-process (mint id, tag project), resolve
...
- [ ] 3.3 Implement the Relay Settings `Configurable`: CRUD for start-script configs and the
      per-event sound toggles (both on by default); persistence round-trip test.

## 4. Agent Sessions tool window

- [ ] 4.1 Implement the dedicated Agent Sessions tool window (factory + panel) rendering from
...
- [ ] 4.2 Host each live session's terminal in the tool window via
...
- [ ] 5.1 Implement the session notifier: IDE notification on `turn.completed`
      ...
      is local-only.
- [ ] 5.2 Play a short beep (`Toolkit.getDefaultToolkit().beep()`) on `turn.completed` and
      `needs.input`, each gated by its Settings toggle; test the toggle gating (pure decision
      function).

## 6. Wiring, docs & verification

- [ ] 6.1 Register all new extensions/services/topics (+ the terminal `<depends>`) in
      `plugin.xml`; run the full test suite and `buildPlugin`.
- [ ] 6.2 End-to-end check without a real agent: in `runIde`, add a `local` start-script
...
- [ ] 6.3 Write `docs/ADAPTERS.md` (integration contract: env vars, normalized webhook,
