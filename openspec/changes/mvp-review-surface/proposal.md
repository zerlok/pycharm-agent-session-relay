...
- **Author comments** on a selection: a keybound editor action grabs the current file + selection range (or caret line) and opens a popup to capture a comment, stored against a subject (path, line range, anchor text, context hash, body). The comment model leaves the subject open so whole-file, batch-level, and multi-file subjects are not foreclosed, but authoring them is deferred (see out of scope).
- **See comments**: gutter icons mark commented lines; a tool window lists pending comments grouped by file with navigate-to-line.
- **Manage comments**: delete pending comments from the gutter or the tool window.
...
```
 ...
 2. open a file, select lines, comment → popup → stored comment
 3. see / delete          → gutter icons + tool window, kept in sync with the store
 4. open tool window      → comments grouped by file; add more
...
- **Whole-file and batch-level comment subjects** — a comment on a file as a whole (`@path`, no range) and detached general feedback on the batch (a top-level note). The subject model allows both; authoring + export are deferred.
- **Multi-file comments** — authoring one comment that spans several files. The model allows it; the authoring/UX is deferred.
- **In-place comment editing** — editing the body of an existing pending comment. The MVP supports delete (then re-add); in-place edit is deferred.
- **Live export preview** — the in-tool-window preview of `REVIEW.md`. The Exporter is a pure function, so the preview slots in later behind the same seam without diverging from the written file.
...
### New Capabilities
- `review-annotation`: Create, display, and delete line-anchored comments on any file in the project — keybound authoring on a selection, gutter markers, and a tool window listing comments grouped by file with navigate-to-line.
- `review-delivery`: Batch the pending comments, export them to `REVIEW.md` in Claude-format markdown (`@path#L` references), and notify the user the file is ready to hand to the idle session.
