...
- [ ] 7.1 **DATA LOSS — `REVIEW.md` already open in an editor.** Reported verbatim: "when REVIEW.md is opened in IDE — export doesn't change it content, so new comments just got lost."
  **Diagnosis (confirmed by reading the code, not reproduced here).** `ReviewDeliveryService.write` (`:174`) calls `Files.writeString`, a raw nio write that bypasses the platform's Document layer, then `LocalFileSystem.refreshAndFindFileByNioFile`. When `REVIEW.md` is open, `FileDocumentManager` holds an in-memory `Document` for it. A VFS refresh from a background thread does **not** reload that Document — reloading must happen on the EDT, and the platform will not clobber a loaded document from a background refresh. So the editor keeps showing the old text.
  That alone would be a display bug. What makes it **data loss** is the combination: the write succeeded, so stage 3 **cleared the batch**, and the stale in-memory document can later be saved back over the file — erasing the export. The comments are then gone from the store *and* absent from the file, with no undo.
  **Why no test caught it:** nothing in the suite opens `REVIEW.md` in an editor. The existing tests assert on the file's bytes, which are genuinely correct on disk; the bug lives entirely in the platform's view of the file.
  **Proposed fix:** keep the off-EDT disk write (the `review-delivery` spec requires the write off the EDT), and in **stage 3 — already on the EDT** — reload the document if one is loaded for that path, via `FileDocumentManager.reloadFiles(vFile)`. One call in a stage that already exists; no threading change. Add a test that opens `REVIEW.md` before submitting.
  **Decision needed:** `reloadFiles` discards unsaved user edits to `REVIEW.md`. For a generated artifact that is probably right, but it should be chosen deliberately rather than happening as a side effect.
