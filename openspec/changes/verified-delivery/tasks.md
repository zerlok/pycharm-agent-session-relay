...
- [ ] 5.5 Manual QA to re-run the original repro: comment on a line, close the file, edit it externally so the commented lines change, Submit ⇒ `REVIEW.md` carries `⚠️ unverified anchor`. Then the orphan variant: delete enough lines that the recorded range no longer exists ⇒ `⚠️ anchor deleted`. **Not run — `runIde` is unavailable in this environment (no display); this is unverified.** The closest automated stand-in is `ReviewDeliveryServiceTest.test the written artifact flags a drifted closed-file comment` / `…flags an orphaned comment`, which drive the whole pipeline and assert on the written `REVIEW.md`, but they do not exercise a real IDE.
