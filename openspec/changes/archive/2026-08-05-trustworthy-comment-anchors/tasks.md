...
- [x] 8.6 Record the two `runIde` checks this environment cannot run as manual QA. **Run by the maintainer on 2026-08-06; results below.**
  - [x] 8.6a Split a commented file into two editor splits. **Observed:** cards render in both splits and the hover "+" follows the pointer only — no visible regression. *Recorded honestly: this cannot confirm the fix's actual claim. Two highlighters at the same offsets in the same color are visually identical to one, so the duplicate-marker bug was never observable by eye. The one-marker-per-document evidence is `DocumentReviewMarkersTest`, which counts highlighters and covers open-two / close-one / close-both.*
  - [x] 8.6b Author a comment, rewrite the file on disk, then Submit. **Run in two variants, and the pair is more informative than either alone:**
    - *File left OPEN during the external edit:* on returning to the IDE the file reloaded and the comment **moved to the correct lines by itself**. This shows the platform's reload-from-disk is **diff-based**, remapping `RangeMarker`s properly — it is not the naive-offset reapply the review report assumed. Consequence to carry forward: the deferred `DocumentTracker` adoption is a much lower priority than the review implied, because the file-open path already works.
    - *File CLOSED during the external edit:* comment on line 9, file closed, lines 2–5 removed externally, then exported. **`REVIEW.md` referenced line 9 with no flag**, now pointing at former line 13. **This change does not cover the closed-file case** — `validateAnchors` needs a live marker, so a comment with no open editor is skipped (design D6 plus the accepted Risk "a closed file's comments are never verified"). That assumption is wrong for Relay's premise, where the agent edits files the user is not looking at. Carried into `verified-delivery` as its primary defect.
...
- [ ] 9.2 Note in the delivery-layer change that it must carry the `validateAnchors()` call along with the flush when it moves the pipeline into `delivery/`. *(Superseded and absorbed: that change now exists as `verified-delivery`, which does more than move the call — it replaces the marker-based verification entirely.)*

## 10. Defects this change shipped with, found in maintainer QA (2026-08-06)

Recorded here rather than fixed, because both are carried by `verified-delivery`. Archiving does not
claim they are closed.

- [ ] 10.1 **A closed file's comments are never verified.** `validateAnchors` reads current anchor text off the live marker (`live[comment.id] ?: continue`), so a comment whose file has no open editor exports `ACTIVE` and unflagged however much the file changed on disk. Reproduced in 8.6b. This was an explicitly accepted Risk in `design.md`; the QA proved the acceptance wrong on first real use.
- [ ] 10.2 **An `ORPHANED` comment exports with no flag.** `ReviewExporter.flagOf` flags `STALE` only, and nothing filters `ORPHANED` out — so a comment whose recorded range no longer exists (e.g. `L300-301` of a since-truncated file) ships as a clean-looking `@file#L300-301`. The `review-export` delta in this change never mentioned `ORPHANED` either, so the implementation matched an incomplete spec. `ORPHANED` was fully defined for the editor and the tool window and then ignored at the one surface that talks to the agent.
