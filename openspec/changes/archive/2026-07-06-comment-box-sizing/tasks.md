...
- [x] 1.2 Verify in the running IDE that constraining the inner panel's max width under
      `fullWidth = true` visibly caps the box; if not, switch that surface to `fullWidth = false` with
      an explicit width per design's fallback. (Confirmed by the user: box and card cap correctly and a
      ~80-column base width applies; inner-panel constraint kept, no `fullWidth = false` fallback needed.)
...
- [x] 5.1 Manually verify in the running IDE: box and card cap at the right margin on a wide editor;
      full-width fallback when the guide is disabled; a 1-line card shows no button row at rest and
      reveals Edit/Delete on hover; a short authoring box leaves more code visible. (Confirmed by the
      user, including newline growth in the box editor without lag.)
