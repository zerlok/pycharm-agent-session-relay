...
The card SHALL be sized to the width the editor actually has. It SHALL never render wider than the
editor's available content width — the viewport less its vertical scrollbar and the gutter — and
SHALL additionally be capped at a comfortable reading measure, and at the editor's configured right
margin (its vertical guide column) when one is set. That sizing SHALL **track** the editor rather
than being fixed when the card is built: when the editor's available width changes — a split, a
window resize, a change to the editor font size or the right-margin setting — the card SHALL be
re-sized to the new width. The card's height SHALL fit its header row plus its body **at the width
the card currently has**, so a short comment still reserves little vertical space and narrowing the
card re-wraps its body and grows it taller rather than clipping the text. The cards SHALL be
...
#### Scenario: Card is capped at a reading measure without a right margin
...
- **THEN** the card widens again, up to its reading measure and right-margin caps, rather than staying
