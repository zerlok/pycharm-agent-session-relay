...
## Purpose
Govern the JetBrains Marketplace listing (the `plugin.xml` `<description>` and its metadata) as a
tracked artifact: what it must contain, at what altitude, and the rule that its behavior claims stay
consistent with — and derived from — the behavior specs rather than restating them.
## Requirements
