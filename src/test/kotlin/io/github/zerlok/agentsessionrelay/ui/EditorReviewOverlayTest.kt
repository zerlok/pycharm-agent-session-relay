 ...
 * reference.
 *
 * Document-scoped ownership across splits, orphaning, and anchor validation are exercised in
 * [DocumentReviewMarkersTest], which drives the real [EditorReviewOverlayService] lifecycle.
 */
class EditorReviewOverlayTest : BasePlatformTestCase() {

    private lateinit var service: ReviewBatchService
    private lateinit var markers: DocumentReviewMarkers
    ...
    override fun setUp() {
        ...
        // A live editor over a real file; its document markup is what the markers project onto.
        myFixture.configureByText("a.py", "line0\nline1\nline2\nline3\nline4\n")
        url = FileDocumentManager.getInstance().getFile(myFixture.editor.document)!!.url

        // Assembled here exactly as EditorReviewOverlayService assembles them: one marker owner per
        // document, handed to the per-editor overlay.
        markers = DocumentReviewMarkers(project, myFixture.editor.document)
        Disposer.register(testRootDisposable, markers)
        overlay = EditorReviewOverlay(project, myFixture.editor, markers)
    ...
    fun `test currentPositions reports the shifted range after an in-IDE edit above the comment`() {
        ...
        assertEquals(Subject.LineRange(url, 1, 2), markers.currentPositions()[comment.id])
        ...
        assertEquals(Subject.LineRange(url, 3, 4), markers.currentPositions()[comment.id])
    }
    ...
    fun `test currentPositions keeps a single-line comment as Line after a shift`() {
        ...
        assertEquals(Subject.Line(url, 1), markers.currentPositions()[comment.id])

        WriteCommandAction.runWriteCommandAction(project) {
            myFixture.editor.document.insertString(0, "top0\ntop1\n")
        }

        assertEquals(Subject.Line(url, 3), markers.currentPositions()[comment.id])
    }

    /**
     * A store subject change (the user re-editing a comment's range) repositions the live marker, so
     * the marker read via [DocumentReviewMarkers.currentPositions] reports the NEW range — not the stale
     ...
     * read of one marker. Both halves are asserted together here, because it is their *agreement* that
     * the anchor check depends on: two separate queries could straddle a document change and validate
     * one range's text against another range's position.
     */
    fun `test liveState reports the current position and the current text of the same range`() {
        val comment = service.addComment(Subject.LineRange(url, 1, 2), "extract this")

        val before = markers.liveState().getValue(comment.id)
        assertEquals(Subject.LineRange(url, 1, 2), before.subject)
        assertEquals("line1\nline2", before.text)

        // An edit ABOVE moves the marker without touching the text it spans.
        WriteCommandAction.runWriteCommandAction(project) {
            myFixture.editor.document.insertString(0, "top0\ntop1\n")
        }

        val after = markers.liveState().getValue(comment.id)
        assertEquals(Subject.LineRange(url, 3, 4), after.subject)
        assertEquals("line1\nline2", after.text)
    }

    /** An edit *within* the commented lines is what changes the reported text — the drift signal. */
    fun `test liveState reports rewritten text under the comment`() {
        val comment = service.addComment(Subject.Line(url, 1), "note")
        val document = myFixture.editor.document

        WriteCommandAction.runWriteCommandAction(project) {
            document.replaceString(document.getLineStartOffset(1), document.getLineEndOffset(1), "rewritten")
        }

        assertEquals("rewritten", markers.liveState().getValue(comment.id).text)
    }

    /** currentPositions is a projection of liveState, so the two can never disagree about position. */
    fun `test currentPositions projects liveState`() {
        val a = service.addComment(Subject.LineRange(url, 1, 2), "a")
        val b = service.addComment(Subject.Line(url, 3), "b")

        assertEquals(
            markers.liveState().mapValues { (_, anchor) -> anchor.subject },
            markers.currentPositions(),
        )
        assertEquals(setOf(a.id, b.id), markers.currentPositions().keys)
    }
